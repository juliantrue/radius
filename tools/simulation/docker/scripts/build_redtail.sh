...
if [[ ! -L "${CATKIN_WS}/src/caffe_ros" ]]; then
    cp -r ${REDTAIL_PATH}/ros/packages/caffe_ros ${CATKIN_WS}/src/
    catkin_make caffe_ros_node -DCUDA_USE_STATIC_CUDA_RUNTIME=OFF
fi

if [[ ! -L "${CATKIN_WS}/src/image_pub" ]]; then
    cp -r ${REDTAIL_PATH}/ros/packages/image_pub ${CATKIN_WS}/src/
fi

if [[ ! -L "${CATKIN_WS}/src/px4_controller" ]]; then
    cp -r ${REDTAIL_PATH}/ros/packages/px4_controller ${CATKIN_WS}/src/
fi
