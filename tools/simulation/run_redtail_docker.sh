...
if [ -z "${REDTAIL_ID}" ]; then
    ...
    xhost +
    nvidia-docker run -it --privileged --network=host -v ${HOST_DATA_DIR}:${CONTAINER_DATA_DIR}:rw -v /tmp/.X11-unix:/tmp/.X11-unix -e DISPLAY=unix${DISPLAY} -p 14556:14556/udp --name=${REDTAIL_NAME} redtail-sim:kinetic-v2 bash
else
